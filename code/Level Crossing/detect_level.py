...
from turtlebot3_autorace_level_crossing_detect.cfg import DetectLevelParamsConfig
...
class DetectLevel():
    def __init__(self):
        self.hue_red_l = rospy.get_param("~detect/level/red/hue_l", 168)
        self.hue_red_h = rospy.get_param("~detect/level/red/hue_h", 179)
        self.saturation_red_l = rospy.get_param("~detect/level/red/saturation_l", 43)
        self.saturation_red_h = rospy.get_param("~detect/level/red/saturation_h", 255)
        self.lightness_red_l = rospy.get_param("~detect/level/red/lightness_l", 39)
    ...
    def fnFindRectOfLevel(self, mask):
        ...
        if len(keypts) == 3:
            ...
            if is_rects_linear == True or is_rects_dist_equal == True:
                ...
                # publishing topic
                self.stop_bar_count = 4
                if distance_bar2car > 0.5:
